...
public:
    ...
    bool change_record_status(size_t index, estatus_t status, payload_t payload)
    {
        ...
        if (unsafe_locate_record(index, &this_record))
        {
            ...
            else
            {
                ...
                link_record_to_queue_end(this_record);                
    ...
    {
        _records_lock.lock();        
        std::fill(&_records[0], &_records[ARRAY_SIZE(_records)], storage_record_t(nullptr));
        _storage_size = 0;
        _storage_top = 0;
        reset_queue_service_data();
        _records_lock.unlock();
...
private:    
    ...
    void link_record_to_queue_end(storage_record_t *record)
    {
        ASSERT(record != nullptr && record != *_pp_queue_last);
        *_pp_queue_last = record;
        _pp_queue_last = &record->_next_record;
    }
